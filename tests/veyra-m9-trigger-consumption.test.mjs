...
const artboard = () => ({ id: 'art', name: 'Art', x: 0, y: 0, width: 640, height: 480, background: '#fff' });
...
function dataDoc(machine, extra = {}) {
  const box = createNode('rectangle', { id: 'box', opacity: 1 });
  const opacity = nodePropertyAddress('box', 'opacity');
  const timelines = [timeline('idle', opacity, .2), timeline('active', opacity, .8)];
...
// A data trigger that participates in a losing candidate is conserved.
{
  ...
  assert.equal(dataRuntime.getValue('inst', 'pulse'), true, 'failed candidate must not consume its pulse');
...
// Exit Time / after gates do not even spend a queued event until the transition becomes eligible.
{
  const machine = singleLayerMachine({
    after: .25,
    exitTime: { unit: 'seconds', value: .5 },
...
// M8 trigger queues preserve extra pulses: one selected transition spends one event only.
{
...
// Duplicate fired conditions referencing the same endpoint spend only one queued pulse.
{
  const machine = singleLayerMachine({ conditions: [
...
// Legacy machine triggers now follow the same exact policy: unmatched pulses persist,
// and a selected transition consumes them rather than step() globally clearing them.
{
  ...
  runtime.step(.01);
  assert.equal(runtime.inputs.find(input => input.id === 'go_input').value, true, 'failed legacy candidate must conserve pulse');
  runtime.setInput('ready_input', true);
  runtime.step(.01);
  assert.equal(runtime.stateId, 'active_state');
  assert.equal(runtime.inputs.find(input => input.id === 'go_input').value, false);
}

// One legacy pulse cannot be reused by a later layer in the same deterministic step.
{
...
// M8 queued trigger counts are deterministic across layers: one pulse wakes one layer;
// two queued pulses allow both ordered layers to transition in the same step.
{
...
// Observation stays non-consuming: a fork may take the transition using its private
// data-runtime queue without spending the live pulse or advancing the live machine.
{
  const machine = singleLayerMachine({ conditions: [{ id: 'pulse_cond', source: data('inst', 'pulse'), op: 'fired' }] });
  const doc = dataDoc(machine);
  const dataRuntime = createVeyraDataRuntime(() => doc);
  ...
  assert.equal(runtime.stateId, 'idle_state');
  assert.equal(dataRuntime.getValue('inst', 'pulse'), true, 'fork must consume only its private queue');
  runtime.step(.01);
  assert.equal(runtime.stateId, 'active_state');
  assert.equal(dataRuntime.getValue('inst', 'pulse'), false);
}

console.log('Veyra M9 exact trigger consumption tests passed');
