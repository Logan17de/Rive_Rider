// Permanent M8-C4 reproduced contracts. Installed and executed RED before production edits.
// Seven independent C3-review regressions plus both positive controls; public APIs only.
...
console.log(`M8-C4 reproduced contracts: ${results.filter(r=>r.status==='PASS').length}/${results.length} passed`);
assert.equal(results.filter(r=>r.status!=='PASS').length,0,JSON.stringify(results.filter(r=>r.status!=='PASS'),null,2));
