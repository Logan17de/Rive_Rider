...
|---|---|---|
| **KD-1** | open, tracked | `renderer.js:436-465` gives every path vertex two bezier handles **unconditionally**; a zero-offset path still exposes six invisible `pointerdown` targets that turn corners into curves. Demonstrated by the renderer-interaction suite; fix needs a decision (hide handles at zero offset, or make `isAnimatable`/hit-testing agree). `renderer.js` is **locked** until assigned. |
...
| **Listeners runtime = 0%** | open | The 3B contract requires (a) a listener **resolver** with stable `pointerenter/leave` (no re-hit-test per `pointermove`, hover-key guard), (b) listeners as a **separate evaluation `input` layer** — `VEYRA_EVALUATION_ORDER` has **no input layer**, so `interactive` is authored-only today, (c) `hitTestPoint`: `capabilities.js` publishes hit-testing capability while **no implementation exists**. Until all three land, loading a listener-bearing v4 file yields a **valid, silent, inert machine** — worse for AI than a hard failure. |
...
2. **Listeners completion**: resolver + `pointerenter/leave` state machine + `hitTestPoint` + the `input` evaluation layer; or, if the runtime slips, **gate the format**: refuse v4 *parse* (message: "requires a Veyra runtime with listener support") so v4 never exists as a silent-inert file.
3. **KD-1** decision + fix + `renderer.js` unlock.
