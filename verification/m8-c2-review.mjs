// Independent verifier probes for production baseline 216deb8c972cf476c3c9c0de90d640e47520568f.
// Intentionally outside tests/: baseline npm test remains unchanged.
// This script reports every result before returning a failing exit code.
...
check('Ownership-advertised Property Group runtime write port must exist in the browser adapter', () => {
  ...
  const port = getOwnership(s.document, 'node:box/opacity', { dataRuntime: runtimeFor(s) }).writableSource.edit.port;
  const source = readFileSync(new URL('../veyra.js', import.meta.url), 'utf8');
  const adapter = source.slice(source.indexOf('globalThis.veyra = Object.freeze({'));
  const declared = new RegExp(`(?:^|\\n)\\s*${port}\\s*:`).test(adapter);
  return { actual: { port, declared }, expected: { port: 'setTwoWayTarget', declared: true } };
});
...
writeFileSync(new URL('./m8-c2-review-results.json', import.meta.url), JSON.stringify({ productionBaseline: BASE, results }, null, 2));
const failed = results.filter(item => item.status !== 'PASS');
console.log(`INDEPENDENT REVIEW: ${results.length - failed.length}/${results.length} passed; ${failed.length} need attention.`);
