...
export function removeViewModelInstanceInDocument(document, instanceId) {
  ...
  document.bindings = (document.bindings || []).filter((binding) => ![binding.source, binding.target].some((endpoint) => endpoint.kind === 'data' && endpoint.instance.id === instanceId));
  document.lists = (document.lists || []).filter((item) => item.owner.id !== instanceId);
  for (const instance of document.viewModelInstances || []) instance.initialValues = instance.initialValues.filter((entry) => !(entry.value?.kind === 'viewModelInstance' && entry.value.id === instanceId) && !(entry.value?.kind === 'list' && ownedLists.has(entry.value.id)));
  document.viewModelInstances = document.viewModelInstances.filter((item) => item.id !== instanceId); return true;
}
...
export class VeyraDataRuntime {
  ...
  removeListItem(listId, itemId, options = {}) { const document = this.#document(); const scope = scopeKey(options.scopePath); const items = this.#runtimeList(document, listId, scope); const before = items.length; this.#lists.set(runtimeListKey(scope, listId), items.filter((item) => item.id !== itemId)); if (before !== items.length) this.#markListDirty(document, listId, scope); return before !== items.length; }
