...
const BASE_PROJECT_CAPABILITIES = Object.freeze([
  ...
  'animation.state-machine-layers',
  'animation.state-machine-layer-priority',
  ...
  'animation.builtin-condition-sources',
  'layout.runtime-evaluation',
  'render.media-svg-bridge',
...
function authoringContract() {
  return {
    dataGraph: {
      ...
      evaluationOrder: ['authored','animation','playback','data-binding','layout','constraints','interactive'],
    ...
    machineLayer: {
      required: ['id', 'enabled', 'weight', 'states', 'transitions'],
      weight: { min: 0, max: 1 },
      evaluation: 'ordered simultaneous clocks; later enabled layers have property priority',
    ...
    machineCondition: {
      ...
      sources: ['machineInput', 'data', 'artboard', 'runtime'],
      builtinValues: {
        artboard: [...VEYRA_MACHINE_BUILTIN_ARTBOARD_VALUES],
        runtime: [...VEYRA_MACHINE_BUILTIN_RUNTIME_VALUES],
      },
